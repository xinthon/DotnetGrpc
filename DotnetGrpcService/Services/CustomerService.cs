...
namespace DotnetGrpcService.Services
{
    public class CustomerService : Customer.CustomerBase
    {
        ...
        public override async Task<CustomerResponse> GetCustomerById(GetCustomerByIdQuery request, ServerCallContext context)
        {
            using (var connection = _factory.CreateConnection())
            {
                CustomerResponse? customer = await connection
                     .QueryFirstOrDefaultAsync<CustomerResponse>("SELECT * FROM Customers WHERE CustomerId = @CustomerId", new { CustomerId = request.CustomerId });
        ...
        public override async Task<CustomerResponse> CreateCustomer(CreateCustomerRequest request, ServerCallContext context)
        {
            using (var connection = _factory.CreateConnection())
            {
                await connection.ExecuteAsync("INSERT INTO Customers(CustomerId, Name, LastName, Email, Address) VALUES(@CustomerId, @Name, @LastName, @Email, @Address)");
        ...
        public override async Task<CustomerResponse> DeleteCustomer(DeleteCustomerRequest request, ServerCallContext context)
        {
            using (var connection = _factory.CreateConnection())
            {
                CustomerResponse? customer = await connection.QueryFirstOrDefaultAsync<CustomerResponse>("SELECT * FROM Customers WHERE CustomerId = @CustomerId", new
                {
        ...
        public override async Task<CustomerResponse> UpdateCustomer(UpdateCustomerRequest request, ServerCallContext context)
        {
            using (var connection = _factory.CreateConnection())
            {
                await connection.ExecuteAsync("UPDATE Customers SET Name = @Name, LastName = @LastName, Email = @Email, Address = @Address WHERE CustomerId = @CustomerId", new
                {
                    CustomerId = request.CustomerId,
                });
